...
export async function routeMessage(
  ...
  // Skip media for now
  if (message.hasMedia) {
    await sendResponse("📎 I can't process media yet. Please describe what you need in text.");
    return;
  }
  ...
  if (queue.size >= config.maxQueueSize) {
    ...
    await sendResponse(`⚠️ Queue full (${config.maxQueueSize} messages). Please wait for current tasks to complete.`);
    return;
  }
  ...
  queue.add(async () => {
    ...
    try {
      ...
      await sendResponse(`❌ Error: ${errorMsg}`);
    }
  });
}
